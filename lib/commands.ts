...
export class DiscordInputError extends Error {
  constructor(message: string) { super(message); this.name = "DiscordInputError"; }
}

function bounded(value: unknown, label: string, min: number, max: number) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed < min || parsed > max) throw new DiscordInputError(`${label} must be between ${min} and ${max}.`);
  return parsed;
}
function positive(value: unknown, label: string, max: number) { return bounded(value, label, Number.MIN_VALUE, max); }
function nonNegative(value: unknown, label: string, max: number) { return bounded(value, label, 0, max); }
...
export async function executeDiscordCommand(name: unknown, options: Record<string, unknown>) {
  ...
  if (name === "perp-risk") {
    ...
    const data = await analyzePerp({ market: requestedMarket, side: requestedSide, risk: profile, accountBalanceUsd: positive(options.balance, "Balance", COMMAND_LIMITS.accountBalance), marginUsd: positive(options.margin, "Margin", COMMAND_LIMITS.margin), leverage: positive(options.leverage, "Leverage", COMMAND_LIMITS.leverage), entryPrice: positive(options.entry, "Entry", COMMAND_LIMITS.price), stopLoss: positive(options.stop, "Stop", COMMAND_LIMITS.price), takeProfit });
